...
> Aim for roughly 35–50% of KPs to include exercises across a module. Quality over quantity.
...
for index, kp in enumerate(kps):
  ...
  slug = slugify(kp.title)  # "Value Projections" → "value-projections"
...
Design each question with **clear conversation path**: Question → Expected Responses → Transition Strategy

```markdown
## Socratic Questions

### Layer 1: Conceptual Understanding (开场问题)
...
**Expected Responses**:
- ✅ Correct: "Sliding window... element-wise multiplication... summing"
  → **Transition**: "Exactly! Now let's dig into *why* we flip the kernel..."
  
- ⚠️ Partial: "Something with matrices?"
  → **Transition**: "You're on the right track! Let me show you a simple example first..."
  
- ❌ Confused: "Not sure..."
  → **Transition**: "No worries! Think about applying a filter to an image - what happens at each pixel?"

**Transition Goal**: Bridge from student's response to Core Definition's "What"

---

### Layer 2: Principle Exploration (深化问题)
...
**Expected Responses**:
- ✅ Correct: "Mathematical definition... signal processing convention..."
  → **Transition**: "Perfect! And here's the practical implication for neural networks..."
  
- ⚠️ Partial: "Maybe for symmetry?"
  → **Transition**: "Good intuition! Let me clarify - it's about how we define the operation. Look at this comparison..."
  
- ❌ Off-track: "To make it faster?"
  → **Transition**: "Interesting thought! Actually, both have same complexity. The real reason is [redirect to principle]..."

**Transition Goal**: Connect student's reasoning to Principles section's "How/Key Components"

---

### Layer 3: Application Scenarios (应用问题)
...
**Expected Responses**:
- ✅ Correct: "Flip the kernel because PyTorch uses cross-correlation..."
  → **Transition**: "Excellent! You've mastered the concept. Let's see this in code..."
  
- ⚠️ Partial: "Something about the kernel orientation?"
  → **Transition**: "You're close! Let me show you the exact pattern... [guide to code example]"
  
- ❌ Unsure: "I'd just use the same weights?"
  → **Transition**: "Let's think through this together. Remember how we said [recap Layer 2]... Now apply that here..."
...
- ✅ **Correct**: Validate + deepen ("Great! Now consider...")
- ⚠️ **Partial**: Affirm + guide ("You're on track! Let's clarify...")  
- ❌ **Confused**: Reassure + simplify ("No worries! Think of it this way...")
...
Missing outline:
```
❌ Outline not found. Run /teacherkit.outline first.
```

Invalid KP structure:
```
❌ Cannot parse KP IDs. Expected format: KP-X.Y.Z
```
